...
print(f"Integrating key point content into course: {course.title}")

# Get all key points for this course
keypoints = KeyPoint.objects.filter(course=course).order_by('title')
print(f"Found {len(keypoints)} key points for this course")

# Create a comprehensive course description that includes all key points
course_description = """# Algorithms and Data Structures
...
# Add a summary of each key point to the course description
for keypoint in keypoints:
    # Just the first paragraph to keep it concise
    first_paragraph = keypoint.description.split('\n\n')[0]
    course_description += f"""### {keypoint.title}
...
course.save()
print("Successfully updated course description")

# Now update each lesson to include the corresponding key point content
for keypoint in keypoints:
    try:
...
print("Key point content has been integrated into the course and lessons for the 'Algorithms and Data Structures' course.") 